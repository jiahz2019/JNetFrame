 ...
 */
void test_fiber1() {
    JHZ_LOG_INFO(g_logger) << "test_fiber1 begin";
  
    /**
     * 协程主动让出执行权，在yield之前，协程必须再次将自己添加到调度器任务队列中，
     * 否则yield之后没人管，协程会处理未执行完的逃逸状态，测试时可以将下面这行注释掉以观察效果
     */
    jhz::Scheduler::GetThis()->schedule(jhz::Fiber::GetThis());

    JHZ_LOG_INFO(g_logger) << "before test_fiber1 yield";
    jhz::Fiber::GetThis()->yield();
    JHZ_LOG_INFO(g_logger) << "after test_fiber1 yield";

    JHZ_LOG_INFO(g_logger) << "test_fiber1 end";
}
 ...
 */
void test_fiber2() {
    JHZ_LOG_INFO(g_logger) << "test_fiber2 begin";
    ...
    JHZ_LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
    JHZ_LOG_INFO(g_logger) << "test_fiber3 begin";
    JHZ_LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
    static int count = 0;

    JHZ_LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
    JHZ_LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
 ...
 */
void test_fiber4() {
    JHZ_LOG_INFO(g_logger) << "test_fiber4 begin";
    
    for (int i = 0; i < 3; i++) {
        jhz::Scheduler::GetThis()->schedule(test_fiber5, jhz::GetThreadId());
    }

    JHZ_LOG_INFO(g_logger) << "test_fiber4 end";
}
...
int main() {
    ...
    jhz::Fiber::ptr fiber(new jhz::Fiber(&test_fiber3));
    sc.schedule(fiber);
